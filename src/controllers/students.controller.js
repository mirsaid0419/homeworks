...
class Students {
  addStudent = (req, res) => {
    try {
      ...
      let allData = JSON.parse(readFileSync(pathStudents, "utf-8"));
      ...
      writeFileSync(pathStudents, JSON.stringify(allData, null, 2));
  ...
  getAllStudents(req, res) {
    try {
      const allData = JSON.parse(readFileSync(pathStudents, "utf-8"));
  ...
  deleteStudent(req, res) {
    try {
      ...
      if (studentIndex != -1) {
        ...
        return res.status(200).json({
          statusCode: 200,
          message: "student success deleted",
        });
      }
      return res.status(404).json({
        statusCode: 404,
